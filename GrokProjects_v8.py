"""
Grok Projects CLI - Website-Parity Coding Companion (Enhanced, v8 for Grok)
====================================================================
...
Usage quickstart:
  ...
  python GrokProjects_v8.py

Batch mode (run instructions from a file):
  python GrokProjects_v8.py my_instructions.txt
...
DEFAULT_MODEL_NAME = "grok-beta"  # Updated default for v8
...
class GrokClient:
    ...
    def chat_with_tools(self, *, model: str, messages: List[Dict[str, Any]], temperature: Optional[float] = 0.2) -> Tuple[str, List[Dict[str, Any]]]:
        """
        Handle chat completion with tool calls (streaming where possible).
        ...
        try:
            ...
            for chunk in response:
                ...
                if getattr(delta, "role", None):
                    # role may appear in first delta
                    pass
                # Stream content
                if getattr(delta, "content", None):
                    response_text += delta.content
                    print(delta.content, end='', flush=True)
                # Accumulate (possibly chunked) tool calls
                if getattr(delta, "tool_calls", None):
                    ...
                    for tc_delta in delta.tool_calls:
                        ...
                        if getattr(tc_delta, "id", None):
                            tool_calls[idx]["id"] += tc_delta.id or ""
            ...
            if has_tool_calls:
                # Append assistant message (streamed content + tool_calls) to conversation
                full_messages.append({"role": "assistant", "content": response_text, "tool_calls": tool_calls})
                ...
                for tc in tool_calls:
                    ...
                    if func_name == "get_current_date":
                        ...
                        try:
                            search_url = f"https://lite.duckduckgo.com/lite/?q={requests.utils.quote(query or '')}"
                ...
                # Recurse to allow the model to use tool results
                return self.chat_with_tools(model=model, messages=full_messages, temperature=temperature)

            # No tool calls; return the streamed text
            return response_text, full_messages

        except Exception as e:
            # Propagate as runtime error so caller can handle
...
class GrokCLI(cmd.Cmd):
    ...
    def __init__(self):
        ...
        self._max_history_len = 20000  # Increased for v8
...
def main():
    print("\n🚀 Starting Grok Projects CLI (Enhanced, v8)\n")
    cli = GrokCLI()

    # If a single positional argument is provided, treat it as an instruction file.
    if len(sys.argv) > 1:
        # Support a common form: python GrokProjects_v8.py my_instructions.txt
