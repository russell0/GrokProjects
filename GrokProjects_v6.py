"""
Grok Projects CLI - Website-Parity Coding Companion (Enhanced, v1 for Grok)
====================================================================
...
- Model management: grok-4 (default), grok-3, grok-3-mini; live `models` list from API
- Streamed responses with automatic non-stream fallback
...
Usage quickstart:
  ...
  python GrokProjects.py

Batch mode (run instructions from a file):
  python GrokProjects.py my_instructions.txt
...
DEFAULT_MODEL_NAME = "grok-4-0709"
# Grok-4 limits (from docs)
MAX_INPUT_TOKENS = 200_000   # Conservative; actual context 256k
...
class GrokClient:
    ...
    def chat_with_tools(self, *, model: str, messages: List[Dict[str, str]], temperature: Optional[float] = 0.2):
        """
        Handle chat completion with tool calls in a loop (non-streaming).
        ...
        while True:
            try:
                ...
                msg = choice.message
                if msg.content:
                    return msg.content, messages  # Return final content and full messages
                ...
                if msg.tool_calls:
                    ...
                    for tc in msg.tool_calls:
                        ...
                        if func_name == "get_current_date":
                            ...
                            # Simple web search using DuckDuckGo lite (no dep)
                            try:
                                ...
                                for link in soup.find_all('a', class_='result-link'):
                                    ...
                                    href = link.get('href')
                                    snippet = link.find_next('td', class_='result-snippet').text.strip() if link.find_next('td', class_='result-snippet') else ""
...
class GrokCLI(cmd.Cmd):
    ...
    def do_model(self, name: str):
        """Switch model by alias or exact id: model <grok4|grok3|mini|model-id>"""
        if not name:
...
def main():
    print("\n🚀 Starting Grok Projects CLI (Enhanced)\n")
    cli = GrokCLI()

    # If a single positional argument is provided, treat it as an instruction file.
    if len(sys.argv) > 1:
        # Support a common form: python GrokProjects.py my_instructions.txt
